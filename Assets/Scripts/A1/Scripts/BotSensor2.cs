    ...
    public class BotSensor2 : Sensor
    {
        public override object Sense()
        {
            ...
            Transform[] tiles1 = FindObjectsOfType<Transform>().Where(t =>
                t.name.Contains("Floor") && t.GetComponent<MyFloor>().State == MyFloor.LightLevel.Light1).ToArray();
            Transform[] tiles2 = FindObjectsOfType<Transform>().Where(t =>
                t.name.Contains("Floor") && t.GetComponent<MyFloor>().State == MyFloor.LightLevel.Light2).ToArray();
            Transform[] tiles3 = FindObjectsOfType<Transform>().Where(t =>
                t.name.Contains("Floor") && t.GetComponent<MyFloor>().State == MyFloor.LightLevel.Light3).ToArray();
